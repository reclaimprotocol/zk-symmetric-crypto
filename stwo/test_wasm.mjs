// Test WASM ChaCha bitwise prover
...
async function main() {
    ...
    console.log("\nTesting bitwise ChaCha prover...\n");

    // Test with log_size=4 (16 blocks = 1KB keystream)
    const log_size = 4;
    const n_blocks = 1 << log_size;
    const keystream_bytes = n_blocks * 64;

    console.log(`Proving ${n_blocks} blocks (${keystream_bytes} bytes)...`);
